...
def add(ctxt, _id, _str, fuzzy):
    ...
    if not fuzzy and _str:
        if ctxt is None:
            ...
            MESSAGES[b"%b\x04%b" % (ctxt, _id)] = _str
...
def make(filename, outfile):  # noqa: C901
    ID = 1
    STR = 2
    CTXT = 3

    # Compute .mo name from .po name and arguments
    infile = filename if filename.endswith(".po") else filename + ".po"
    if outfile is None:
